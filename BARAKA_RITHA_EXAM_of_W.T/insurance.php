...
<form method="post" onsubmit="return confirmInsert();">
    ...
    <input type="number" id="car_model" name="car_model" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO insurance(InsuranceID, VehicleID, Provider, PolicyNumber, ExpirationDate) VALUES (?, ?, ?, ?, ? )");
    $stmt->bind_param("issss", $InsuranceID, $VehicleID, $Provider, $PolicyNumber, $ExpirationDate);
    // Set parameters and execute
    $InsuranceID = $_POST['driver_id'];
    $VehicleID = $_POST['user_id'];
    $Provider = $_POST['license_number'];
    $PolicyNumber = $_POST['car_model'];
    $ExpirationDate = $_POST['capacity'];
...
<body>
    <center><h2>insurance Table</h2></center>
    <table border="3">
        <tr>
            <th>InsuranceID</th>
            <th>VehicleID</th> <!-- corrected closing tag -->
            <th>Provider</th>
            <th>PolicyNumber</th>
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $InsuranceID = $row['InsuranceID']; // Fetch the InsuranceID
        echo "<tr>
            ...
            <td><a style='padding:4px' href='delete_insurance.php?InsuranceID=$InsuranceID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_insurance.php?InsuranceID=$InsuranceID'>Update</a></td> 
        </tr>";
    }
