...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO reviews(ReviewID, UserID,VehicleID,Rating, Comment,DatePosted ) VALUES (?, ?, ?, ?, ?,?)");
    $stfmt->bind_param("isssss", $ReviewID, $UserID, $VehicleID, $Rating, $Comment,$DatePosted );
    // Set parameters and execute
    $ReviewID = $_POST['driver_id'];
    $UserID = $_POST['user_id'];
    $VehicleID = $_POST['license_number'];
    $Rating= $_POST['car_model'];
    $Comment = $_POST['capacity'];
    $DatePosted = $_POST['capacity'];
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $ReviewID = $row['ReviewID']; // Fetch the ReviewID
        echo "<tr>
