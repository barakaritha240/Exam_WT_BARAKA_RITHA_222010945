...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO reservations(ReservationID, UserID,VehicleID,ReservationDate, Status ) VALUES (?, ?, ?, ?, ?)");
    $stfmt->bind_param("issss", $ReservationID, $UserID, $VehicleID, $ReservationDate, $Status );
    // Set parameters and execute
    $ReservationID = $_POST['driver_id'];
    $UserID = $_POST['user_id'];
    $VehicleID = $_POST['license_number'];
    $ReservationDate= $_POST['car_model'];
    $Status = $_POST['capacity'];
...
<body>
    <center><h2>reservations Table</h2></center>
    <table border="3">
        <tr>
            <th>ReservationID</th>
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $ReservationID = $row['ReservationID']; // Fetch the ReservationID
        echo "<tr>
            ...
            <td>" . $row['Status'] . "</td>
            <td><a style='padding:4px' href='delete_vehicles.php?ReservationID=$ReservationID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_vehicles.php?ReservationID=$ReservationID'>Update</a></td> 
        </tr>";
    }
